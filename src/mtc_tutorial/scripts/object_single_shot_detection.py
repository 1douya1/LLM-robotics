...
class SingleShotDetection(Node):
    ...
    def color_callback(self, msg):
        ...
        with self.data_lock:
            try:
                ...
                # Only log on first capture or if verbose logging is enabled
                if not self.first_color_captured:
                    self.get_logger().info("Color image captured")
                    self.first_color_captured = True
    ...
    def depth_callback(self, msg):
        ...
        with self.data_lock:
            try:
                ...
                # Only log on first capture or if verbose logging is enabled
                if not self.first_depth_captured:
                    self.get_logger().info("Depth image captured")
                    self.first_depth_captured = True
                elif self.verbose_logging:
                    self.get_logger().info("Depth image captured")
    ...
    def check_capture_complete(self):
        ...
        if (self.captured_color_image is not None and 
            ...
            # Only log on first complete capture or if verbose logging is enabled
            if not self.first_capture_complete:
                self.get_logger().info("Data capture completed!")
                self.first_capture_complete = True
            elif self.verbose_logging:
                self.get_logger().info("Data capture completed!")
    ...
    def perform_detection(self):
        ...
        for result in results:
            ...
            for box in boxes:
                ...
                object_3d = self.get_object_3d_position(x1, y1, x2, y2, depth_image)
        ...
        try:
            ...
            for obj in detection_result.objects:
                ...
                if obj.class_name == 'cup':
                    # 杯子显示坐标信息
                    if obj.transform_valid:
                        simple_label = f"{obj.class_name}_{display_id} (X:{obj.position_base.x:.3f},Y:{obj.position_base.y:.3f},Z:{obj.position_base.z:.3f})"
                        # 同时在日志中输出坐标
                        self.get_logger().info(f"📍 杯子坐标 {display_id}: X={obj.position_base.x:.3f}, Y={obj.position_base.y:.3f}, Z={obj.position_base.z:.3f}")
                    else:
                        simple_label = f"{obj.class_name}_{display_id} (坐标转换失败)"
                        self.get_logger().warn(f"⚠️ 杯子 {display_id}: 坐标转换失败")
                else:
                    # 其他物体仅显示类别和ID
                    simple_label = f"{obj.class_name}_{display_id}"
