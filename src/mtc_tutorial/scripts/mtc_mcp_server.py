...
async def execute_pour(params: Dict[str, Any],
    ...
    2. 然后运行execute_pour执行任务
    3. 可选：使用update_cup_pose更新杯子位置
        
    注意：此工具不再负责场景构建，必须先使用setup_planning_scene工具构建场景。
    """
...
@server.tool()
async def execute_pour_with_timeout(params: Dict[str, Any],
                                    ...
                                    cup_qy: Optional[float] = None,
                                    cup_qz: Optional[float] = None,
                                    cup_qw: Optional[float] = None,
                                    action_name: str = 'execute_pour') -> Dict[str, Any]:
    """执行倒水动作（带强制超时，避免长时间阻塞其他工具） - 重构版本
    
    这是execute_pour的改进版本，专门解决MCP工具调用阻塞的问题：
    - 强制限制最大执行时间，避免长时间阻塞
    - 自动超时取消，释放MCP服务器资源
    - 更适合在需要保持工具响应性的环境中使用
    - 重构后同样需要先运行setup_planning_scene构建场景
    
    Args:
        params: 倾倒参数字典（同execute_pour）
        max_execution_time: 最大执行时间（秒），超时自动取消
        cup_x, cup_y, cup_z: 杯子位置坐标（仅用于位姿更新）
        cup_qx, cup_qy, cup_qz, cup_qw: 杯子方向四元数
        action_name: Action服务器名称
        
    Returns:
        执行结果字典，包含超时处理信息和对象检查结果
        
    注意：重构后此工具也需要先使用setup_planning_scene工具构建场景。
    """
    # 将杯子位姿参数合并到params中（仅用于位姿更新）
    if cup_x is not None and cup_y is not None and cup_z is not None:
        params = {**params,
    ...
    # 使用cancel_after来强制限制执行时间
    result = tools.execute_pour(
        params, 
        action_name=action_name, 
        timeout_sec=max_execution_time,
        cancel_after=max_execution_time * 0.9  # 在90%的时间后取消，留一些缓冲
    )
    
    # 添加超时处理信息
    if result.get("status") == "timeout" or result.get("status") == "canceled":
        result["timeout_handled"] = True
        result["max_execution_time"] = max_execution_time
        result["message"] = f"操作在{max_execution_time}秒后自动取消，避免长时间阻塞其他工具"
...
@server.tool()
async def setup_planning_scene(include_cup: bool = True,
    ...
    """设置完整的规划场景，包括桌面和杯子对象 - 从mtc_tutorial.cpp提取的完整场景构建

    这个工具负责构建完整的MoveIt规划场景，包括：
    1. 台面碰撞对象 (OptoSigma光学面包板)  
    2. 台面支撑结构
    3. 可选的杯子对象（圆柱体）

    Args:
        include_cup: 是否包含杯子对象（默认True）
        cup_x, cup_y, cup_z: 杯子位置坐标（米）
        cup_qx, cup_qy, cup_qz, cup_qw: 杯子方向四元数（可选）
        cup_height: 杯子高度（米，默认0.1）
        cup_radius: 杯子半径（米，默认0.02）
        timeout_sec: 超时时间
    ...
    Returns:
        ...
        - ok: 是否成功
        - status: 操作状态
        - objects_added: 成功添加的对象列表
        - table_objects: 桌面相关对象
        - cup_included: 是否包含杯子
        - cup_position/orientation: 杯子位姿信息
        - msg: 详细消息

    这个工具取代了之前在execute_pour中的场景构建逻辑，
    实现了职责分离：场景构建与任务执行分开管理。
    """
...
async def place_container(target_pose: Optional[Dict[str, float]] = None,
    ...
    Args:
        target_pose: 目标放置位置（可选，使用默认位置）
                     包含 {"x": float, "y": float, "z": float, 可选: "qx", "qy", "qz", "qw"}
        object_id: 目标对象ID
